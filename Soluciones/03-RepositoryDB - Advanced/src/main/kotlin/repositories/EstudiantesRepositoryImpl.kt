...
class EstudiantesRepositoryImpl(
    ...
    override fun findAll(): List<Estudiante> {
        ...
        // Aquí iría la consulta a la base de datos, y rerremos la lista de resultados
        /* db.select(sql).forEach {
             val estudiante = EstudianteEntityDto(
                 id = it["id"] as Long,
                 nombre = it["nombre"] as String,
                 fechaNacimiento = it["fecha_nacimiento"] as String,
                 calificacion = it["calificacion"] as Double,
                 repetidor = it["repetidor"] as Boolean,
        ...
        /*val result = db.select(sql) // Obtenemos los resultados de la consulta
        result.forEach {
            val estudiante = parseRow(it)
            // Añadimos el estudiante a la lista
            estudiantes.add(estudiante)
        }*/
    ...
    override fun findById(id: Int): Estudiante? {
        ...
        /* db.select(sql, id).forEach {
             estudiante = EstudianteEntityDto(
                 id = it["id"] as Long,
                 nombre = it["nombre"] as String,
                 fechaNacimiento = it["fecha_nacimiento"] as String,
                 calificacion = it["calificacion"] as Double,
        ...
        /* val result = db.select(sql, id) // Obtenemos los resultados de la consulta
         result.forEach {
             estudiante = parseRow(it)
         }*/
